...
PSUp1::PSUp1(IPMonster* monster)
{
	m_Monster = monster;
}

void PSUp1::Init(string name)
{
...
void PSUp1::Update(float deltaTime, HitBox* hitBox)
{
	...
	if (m_Animation->getGlobalBounds().contains((sf::Vector2f)sf::Mouse::getPosition(*WConnect->getWindow()))) {
		...
		if (!hitBox->isSkip() && sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
			...
			hitBox->setSkip(true);
			m_Monster->getHitBox()->setPosition(820.f, 825.f);
			m_Animation->setScale(1 / 6.f, 1 / 6.f);
			m_Monster->setPick(true);
			m_Monster->getHitBox()->setTag(PLAYER);
...
void PSUp1::Render(sf::RenderWindow* window)
{
